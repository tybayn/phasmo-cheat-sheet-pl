...
var user_settings = {"num_evidences":3,"ghost_modifier":2,"volume":50,"mute_timer_toggle":0,"mute_timer_countdown":0,"offset":0,"sound_type":0,"speed_logic_type":0,"bpm":0,"domo_side":0,"theme":"Domyślny"}
...
function filter(ignore_link=false){
    state["evidence"] = {}
    state["speed"] = {"Wolny":0,"Normalny":0,"Szybki":0}
    ...
    for (var i = 0; i < ghosts.length; i++){
        ...
        if (name != "Mimik" && speed_has_los != -1 && speed_has_los != has_los){
            loskeep = false
        }
        ...
        // Koszmarny Mode
            ...
            if (evi_array.length == 3 && name != "Mimik"){
                keep = false
            }
        ...
        // Insanity
            ...
            if (evi_array.length == 2 && name != "Mimik"){
                keep = false
            }
            else if (evi_array.length > 0){
        ...
        // Apokalipsa
            ...
            if (evi_array.length > 0 && name != "Mimik"){
                keep = false
            }

            if (not_evi_array.length > 0 && name == "Mimik"){
                keep = false
            }
        }
        ...
        if (spe_array.length > 0){
            ...
            var shas = (min_speed < base_speed || name == "Mimik")
            var nhas = (speed_type == "or" && (min_speed === base_speed || max_speed === base_speed || name == "Mimik")) || (speed_type == "range" && min_speed <= base_speed && base_speed <= max_speed)
            var fhas = (max_speed > base_speed || name == "Mimik")
        ...
        if (keep && loskeep){
            if(min_speed < base_speed || name == "Mimik"){
                keep_speed.add('Wolny')
                if (marked_not)
                    fade_speed.add('Wolny')
                else
                    not_fade_speed.add('Wolny')
            }
            if ((speed_type == "range" && min_speed <= base_speed && base_speed <= max_speed) || name == "Mimik"){
                keep_speed.add('Normalny')
                if (marked_not)
                    fade_speed.add('Normalny')
                else
                    not_fade_speed.add('Normalny')
            }
            ...
            if(max_speed > base_speed || name == "Mimik"){
                keep_speed.add('Szybki')
                if (marked_not)
                    fade_speed.add('Szybki')
                else
                    not_fade_speed.add('Szybki')
            }

            if(sanity[0] > san_lookup['Niski'] || sanity[1] > san_lookup['Niski']){
                keep_sanity.add('Niski')
                if (marked_not)
                    fade_sanity.add('Niski')
                else
                    not_fade_sanity.add('Niski')
            }
            ...
            if(sanity[0] > san_lookup['Bardzowysoki'] || sanity[1] > san_lookup['Bardzowysoki']){
                ...
                if (marked_not)
                    fade_sanity.add('Bardzowysoki')
                else
                    not_fade_sanity.add('Bardzowysoki')
            }
        }
    ...
    if (num_evidences == "3"){
        ...
        all_evidence.filter(evi => evi != 'Orb').forEach(function(item){
            var checkbox = document.getElementById(item);
            $(checkbox).addClass("block")
            $(checkbox).find("#checkbox").removeClass(["good","bad","faded"])
            $(checkbox).find("#checkbox").addClass(["neutral","block","disabled"])
...
function resetResetButton(){
    ...
    $("#reset").html(polled ? "Oczekiwanie na pozostałych..." : "Resetuj")
...
function flashMode(){
    ...
    var mode_text = ["Apokalipsa","Insanity","Koszmarny","Profesjonalista"][cur_evidence]
    document.getElementById("game_mode").innerHTML = `${mode_text}<span>(${cur_evidence} dowody)</span>`
...
function loadSettings(){
    ...
    try{
        ...
        user_settings = {"num_evidences":3,"ghost_modifier":2,"volume":50,"mute_timer_toggle":0,"mute_timer_countdown":0,"offset":0,"sound_type":0,"speed_logic_type":0,"bpm_type":0,"bpm":0,"domo_side":0,"theme":"Domyślny"}
    }
