...
const all_speed = {"Slow":"Wolny","Normal":"Normalny","Fast":"Szybki"}
const all_sanity = {"Late":"Niski","Average":"Average","Early":"Wysoki","VeryEarly":"Bardzowysoki"}
let all_evidence = []
let all_ghosts = []
let all_maps = {}
let bpm_list = []
...
var user_settings = {"num_evidences":"3","cust_num_evidences":"3","cust_hunt_length":"3","ghost_modifier":2,"volume":50,"mute_timer_toggle":0,"mute_timer_countdown":0,"timer_count_up":0,"timer_split":1,"offset":0.0,"sound_type":0,"speed_logic_type":0,"bpm":0,"domo_side":0,"priority_sort":0,"map":"6 Tanglewood Drive","theme":"Domyślny"}
...
function closeMenu(){
    ...
    if(mquery.matches){
        document.getElementById("menu").style.marginBottom = is_c ? "-705px" : "-650px";
...
function flashMode(){
    ...
    var mode_text = {"-1":"Niestandardowe","0":"Apokalipsa III","1":"Obłęd","2":"Koszmarny","3":"Profesjonalista","3I":"Średni","3A":"Amator"}[cur_evidence]
    document.getElementById("game_mode").innerHTML = `${mode_text}<span>(${parseInt(cur_evidence)} dowody)</span>`.replace("-1",document.getElementById("cust_num_evidence").value)
...
function loadSettings(){
    ...
    try{
        ...
        user_settings = {"num_evidences":"3","cust_num_evidences":"3","cust_hunt_length":"3","ghost_modifier":2,"volume":50,"mute_timer_toggle":0,"mute_timer_countdown":0, "timer_count_up":0,"timer_split":1,"offset":0.0,"sound_type":0,"speed_logic_type":0,"bpm_type":0,"bpm":0,"domo_side":0,"priority_sort":0,"map":"6 Tanglewood Drive","theme":"Domyślny"}
    }
...
function showCustom(){
    ...
    if(document.getElementById("num_evidence").value == "-1"){
        if(mquery.matches){
            document.getElementById("menu").style.height="740px";
            if(is_h){
                document.getElementById("menu").style.marginBottom = "-705px";
                $("#domovoi").removeClass("domovoi-custom")
                $("#domovoi").addClass("domovoi-menu-hidden")
            }
            else{
        ...
        if(mquery.matches){
            if(is_h){
                document.getElementById("menu").style.marginBottom = "-650px";
                $("#domovoi").removeClass("domovoi-custom")
                $("#domovoi").addClass("domovoi-menu-hidden")
            }
            else{
            ...
            document.getElementById("menu").style.height="685px";
        }
    }
}
